...
- 6단계 인터뷰(3 Phase): 생활 환경 → 반복 소모 → **OS Core** → 부품 지도 → 첫 부품 결정 → 결과물.
- **OS Core** = 내 OS가 지켜야 할 핵심 구조/가치 한 문장(예: *원고 하나로 모든 SNS에 자동 발행되는 구조*). 결과물에는 **유형 배지**(개인 / 삶 / 업무›세부)도 붙습니다.
...
Claude Code 채팅에 아래 문구를 **그대로 붙여넣으세요.** 나머지는 Claude가 알아서 설치합니다.

```
https://github.com/hapum0225/personal-os 이 스킬을 사용할 수 있는 상태로 세팅해줘
```

> 설치 중 Claude가 "출처를 확인해달라"거나 "새 세션을 열어달라"고 하면, 안내하는 대로 따라 하면 됩니다.

### 2단계 · 인터뷰 시작

설치가 끝나면 (필요하면 새 채팅을 열고) 아래처럼 시작하세요.
...
- **40~60분** 동안 Claude가 6단계로 하나씩 물어봅니다. 정답은 없으니 편하게 답하세요.
- 중간에 끊겨도 괜찮습니다 — 같은 폴더에서 다시 시작하면 **"이어서 할까요?"** 라고 물어봅니다.
- 끝나면 `os-output/` 폴더에 **OS 청사진 1장 + 첫 부품 결과물 1개**가 파일로 생깁니다.

### 잘 안 될 때

- 설치했는데 발동이 안 되면 → **새 Claude Code 세션**을 열거나, 채팅에 `/reload-plugins` 를 입력하세요.
- 그래도 안 되면 → 위 **1단계 설치 문구**를 다시 붙여넣어 보거나, 강사에게 문의하세요.
